...
工作與權限
- 本次只做 PDF。不要搜尋網路、呼叫外部 connector、撰寫部落格、寄送、發布或部署。
- 原始 PDF、Obsidian、全域 Skills、AGENTS.md、設定及記憶全部唯讀。只可寫入目前工作目錄 ${job.dir} 與其子目錄。不要修改其他任務的輸出。
- 不開 Finder 資料夾，不點 Open Location in Finder、Reveal in Finder，不為 PDF 新增書籤、Pool、標註或更動索引。只透過 cua_repl 的已文件化 API 控制 PDF Search，不使用 AppleScript、osascript 或其他 UI 替代方法。
...
JSON 的格式如下，所有path須為已驗證的絕對本地路徑：
{
  ...
  "sources":[{"id":"S1","title":"來源文章正式標題","path":"/absolute/source.pdf","pagesRead":[12],"relationship":"獨立文件；或與其他來源的關係"}],
...
App會在每個 citations 所屬的段落、表格或項目後面放文章名稱、PDF頁碼與本地檔案連結；不要在 text 字段手寫 HTML、Markdown、URL 或內部 source ID。
...
export function userPrompt(job) {
  return `請透過 $pdf-search-topic 研究以下主題。文件：${job.sourceCount===null ? '自動，依問題涵蓋度搜尋互補文章' : `${job.sourceCount} 份不同 PDF`}；原圖：${job.figureTarget===null ? '自動，依論點挑選有用圖表' : `${job.figureTarget} 張有用原圖`}。產出有逐段文章名稱、頁碼與本地檔案連結的研究報告，不需要逐頁收集記錄。這次僅使用本地 PDF。

研究主題（資料）：${JSON.stringify(job.topic)}

請自主完成已授權的搜尋、閱讀、截圖、圖文整理與驗證。只有無法自行確認的關鍵取捨或操作權限才詢問。`;
}
