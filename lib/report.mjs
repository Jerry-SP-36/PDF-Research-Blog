...
export async function validateReport(input, options = {}) {
  const { jobDir, allowedSourceRoots = [], sourceTarget = 3, figureTarget = 8, requireEvidence = true, allowWebSources = false } = options;
  ...
  if (typeof allowWebSources !== 'boolean') fail('allowWebSources: expected boolean');

  const sourceMap = new Map(), figureMap = new Map(), usedPages = new Set(), usedSourceIds = new Set(), referencedFigures = new Set();
  for (const [index, value] of array(data.sources, 'sources', 1).entries()) {
    const where = `sources[${index}]`, item = record(value, where);
    const kind = item.kind === undefined ? 'pdf' : string(item.kind, `${where}.kind`);
    if (!['pdf', 'web'].includes(kind)) fail(`${where}.kind: expected pdf or web`);
    let source;
    if (kind === 'web') {
      if (!allowWebSources) fail(`${where}: web sources are not allowed for this job`);
      source = { id: identifier(item.id, `${where}.id`), kind: 'web', title: string(item.title, `${where}.title`), url: string(item.url, `${where}.url`), accessedAt: string(item.accessedAt, `${where}.accessedAt`) };
      try {
        const url = new URL(source.url);
        if (url.protocol !== 'https:' || url.username || url.password) throw new Error('must be an HTTPS URL without credentials');
        source.url = url.href;
      } catch (error) { fail(`${where}.url: ${error.message}`); }
      if (!Number.isFinite(Date.parse(source.accessedAt))) fail(`${where}.accessedAt: expected an ISO date or timestamp`);
      else source.accessedAt = new Date(source.accessedAt).toISOString();
      if (item.publishedAt !== undefined) {
        source.publishedAt = string(item.publishedAt, `${where}.publishedAt`);
        if (!Number.isFinite(Date.parse(source.publishedAt))) fail(`${where}.publishedAt: expected a date or timestamp`);
      }
    } else {
      source = { id: identifier(item.id, `${where}.id`), kind: 'pdf', title: string(item.title, `${where}.title`), path: string(item.path, `${where}.path`), pagesRead: pages(item.pagesRead, `${where}.pagesRead`) };
    }
  ...
  function checkPage(sourceId, page, location, used = true) {
    ...
    if (source.kind !== 'pdf') { fail(`${location}: ${sourceId} is a web source and cannot have PDF pages`); return false; }
    if (!positive(page) || !source.pagesRead.includes(page)) { fail(`${location}: page ${page} is not within ${sourceId}.pagesRead`); return false; }
    if (used) { usedPages.add(`${sourceId}:${page}`); usedSourceIds.add(sourceId); }
    return true;
  }
  function citations(value, location) {
    ...
    return items.map((value, index) => {
      ...
      const sourceId = identifier(item.sourceId, `${where}.sourceId`), source = sourceMap.get(sourceId);
      if (!source) fail(`${where}.sourceId: unknown source ${sourceId}`);
      let citation;
      if (source?.kind === 'web') {
        if (item.pages !== undefined) fail(`${where}.pages: web citations must not include PDF pages`);
        citation = { sourceId };
        usedSourceIds.add(sourceId);
      } else {
        citation = { sourceId, pages: pages(item.pages, `${where}.pages`) };
  ...
  const sourceHashes = new Map(), validSourceIds = new Set(), validWebSourceIds = new Set(), webUrls = new Map();
  for (const source of report.sources) {
    if (source.kind === 'web') {
      if (webUrls.has(source.url)) fail(`sources.${source.id}: duplicate web URL of ${webUrls.get(source.url)}`);
      else { webUrls.set(source.url, source.id); validWebSourceIds.add(source.id); }
      continue;
    }
    try {
  ...
  for (const source of report.sources) if (!usedSourceIds.has(source.id)) warn(`sources.${source.id}: source is not cited in the report and is excluded from source count`);
  const stats = {
    sources: [...validSourceIds].filter(id => usedSourceIds.has(id)).length,
    webSources: [...validWebSourceIds].filter(id => usedSourceIds.has(id)).length,
    figures: figureHashes.size,
  };
...
const coverageScopeNote = report => report.sources.some(source => source.kind === 'web')
  ? '研究範圍包含本次可用的本地 PDF 與實際開啟的網頁；結束搜尋不代表所有已出版或線上資料均已涵蓋。'
  : '研究範圍僅限本次可用的本地 PDF 來源；結束搜尋不代表所有已出版資料均已涵蓋。';
...
function makeContext(report, jobId) {
  ...
  function htmlCitations(citations = []) {
    return citations.flatMap(citation => {
      ...
      if (!source) return [];
      if (source.kind === 'web') return [`<a class="source-citation" href="${escapeHtml(source.url)}" target="_blank" rel="noopener noreferrer">《${escapeHtml(source.title)}》</a>`];
  ...
  function mdCitations(citations = []) {
    return citations.flatMap(citation => {
      const source = sources.get(citation.sourceId);
      if (!source) return [];
      if (source.kind === 'web') return [`[《${escapeMarkdown(source.title)}》](<${source.url}>)`];
...
export function renderMarkdown(report) {
  ...
  if (report.researchCoverage) lines.push('## 研究範圍', '', escapeMarkdown(report.researchCoverage.summary), '', coverageScopeNote(report), '');
  if (report.limitations.length) lines.push('## 限制與待確認事項', '', ...report.limitations.map(value => `- ${escapeMarkdown(value)}`), '');
  lines.push('## 資料來源', '', ...report.sources.map(source => source.kind === 'web'
    ? `- [《${escapeMarkdown(source.title)}》](<${source.url}>) — 網頁，存取於 ${escapeMarkdown(source.accessedAt.slice(0, 10))}${source.publishedAt ? `；發布／更新 ${escapeMarkdown(source.publishedAt)}` : ''}${source.relationship ? `；${escapeMarkdown(source.relationship)}` : ''}`
    : `- [《${escapeMarkdown(source.title)}》](<${markdownPath(source.path)}>)${source.relationship ? ` — ${escapeMarkdown(source.relationship)}` : ''}`), '', '圖表取自所標示的原始文件，權利屬原作者或權利人。', '');
  return lines.join('\n');
}
...
export function renderHtml(report, { jobId } = {}) {
  ...
  const coverage = report.researchCoverage ? `<section class="research-coverage"><h2>研究範圍</h2><p>${escapeHtml(report.researchCoverage.summary)}</p><p>${escapeHtml(coverageScopeNote(report))}</p></section>` : '';
  ...
  const bibliography = `<section class="bibliography"><h2>資料來源</h2><ul>${report.sources.map(source => source.kind === 'web'
    ? `<li><a href="${escapeHtml(source.url)}" target="_blank" rel="noopener noreferrer">《${escapeHtml(source.title)}》</a> — 網頁，存取於 ${escapeHtml(source.accessedAt.slice(0, 10))}${source.publishedAt ? `；發布／更新 ${escapeHtml(source.publishedAt)}` : ''}${source.relationship ? `；${escapeHtml(source.relationship)}` : ''}</li>`
    : `<li><a href="${context.base}/sources/${encodeURIComponent(source.id)}" target="_blank" rel="noopener" title="${escapeHtml(source.path)}">《${escapeHtml(source.title)}》</a>${source.relationship ? ` — ${escapeHtml(source.relationship)}` : ''}</li>`).join('')}</ul></section>`;
