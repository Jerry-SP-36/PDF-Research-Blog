...
function cleanTopics(value, fallback) {
  ...
  return [...new Set(items.map((item, index) => text(item, `頁面主題[${index}]`, MAX_TOPIC)))];
}

function conciseTopic(value) {
  if (value.length <= MAX_TOPIC) return value;
  return `${value.slice(0, MAX_TOPIC - 1).trimEnd()}…`;
}
  ...
  async captureReport(job, report, sourceRoots) {
    ...
    for (const source of report?.sources || []) {
      ...
      saved.push(await this.record(job, {
        ...
        topics: [conciseTopic(job.topic)], summary, findings, query: '',
