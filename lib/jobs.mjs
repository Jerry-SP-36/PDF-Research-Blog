...
import { ExperienceStore } from './experience.mjs';
import { ReadingMemoryStore } from './reading-memory.mjs';
...
const canonicalHttps = value => { try { const url=new URL(value); return url.protocol==='https:' && !url.username && !url.password ? url.href : null; } catch { return null; } };
...
export function cleanRequest(body, defaults = {}) {
  ...
  const sourceMode = body.sourceMode ?? 'pdf';
  if (sourceCount !== null && (!Number.isInteger(sourceCount) || sourceCount < 3 || sourceCount > 10)) throw new Error('來源份數需為不限或 3–10。');
  ...
  if (!['pdf','pdf_web'].includes(sourceMode)) throw new Error('請選擇有效的資料範圍。');
  return { topic, sourceCount, figureTarget, model, reasoningEffort, sourceMode };
}
...
export class JobManager {
  constructor(config, { rpcFactory = options => new CodexRpc(options), artifactVerifier } = {}) {
    ...
    this.readingMemory = new ReadingMemoryStore(config.dataDir, { pageCounter: async file => {
      const result = await exec(path.join(config.pdfToolDir, 'pdfinfo'), [file], { timeout: 30000, maxBuffer: 1024 * 1024 });
  ...
  async init() {
    ...
    await this.experiences.init();
    await this.readingMemory.init();
    for (const dirent of await readdir(path.join(this.config.dataDir, 'jobs'), { withFileTypes: true })) {
      ...
      try {
        ...
        job.pendingRequests = [];
        job.sourceMode = job.sourceMode === 'pdf_web' ? 'pdf_web' : 'pdf';
        job.stats = { sources: 0, webSources: 0, figures: 0, ...(job.stats || {}) };
        job.runtimeEvidence = { cuaCalls: 0, screenshotCalls: 0, screenshotsSaved: 0, webSearchCalls: 0, webUrls: [], ...(job.runtimeEvidence || {}) };
        job.readingMemoryApplied = {
          records: Number.isSafeInteger(job.readingMemory?.reused) ? job.readingMemory.reused : 0,
          staleCount: Number.isSafeInteger(job.readingMemory?.staleSkipped) ? job.readingMemory.staleSkipped : 0,
        };
    ...
    for (const job of this.jobs.values()) if (!ACTIVE.has(job.status)) {
      await this.experiences.capture(job);
      await this.persist(job);
    }
  }
  ...
  publicJob(job, detailed = true) {
    return {
      ...
      experience: this.experiences.get(job.id),
      experienceApplied: job.experienceApplied || {
        preferences: job.experienceContext?.preferences?.length || 0,
        related: job.experienceContext?.related?.length || 0,
      },
      readingMemory: {
        recorded: this.readingMemory.countForJob(job.id),
        reused: job.readingMemoryApplied?.records || 0,
        staleSkipped: job.readingMemoryApplied?.staleCount || 0,
      },
    };
  }
  ...
  getStatus() {
    const readingMemoryRecords=this.readingMemory.count();
    const checks=(this.status.checks || []).map(check=>check.key==='readingMemory'?{...check,detail:`已保存 ${readingMemoryRecords} 筆按主題與頁碼的閱讀紀錄`}:check);
    return { ...this.status, checks, defaultModel:this.config.defaultModel ?? 'gpt-5.6-luna', defaultReasoningEffort:this.config.defaultReasoningEffort ?? 'high', readingMemoryRecords, activeJobId: this.activeJobId };
  }
  ...
  async performPreflight() {
    ...
    for (const [key,label,file] of [
      ...
      ['pdfInfo','PDF 頁碼工具',path.join(this.config.pdfToolDir,'pdfinfo')],
      ['python','報告驗證工具',this.config.pythonPath],
    ]) {
      try { await access(file); checks.push({ key,label,status:'ok',detail:key==='pdfSearch'?'已安裝；PDF Research 已設為自動允許操作':file }); }
    ...
    checks.push({key:'readingMemory',label:'閱讀記憶',status:'ok',detail:`已保存 ${this.readingMemory.count()} 筆按主題與頁碼的閱讀紀錄`});
  ...
  async enqueue(body) {
    ...
    if ([...this.jobs.values()].filter(j=>ACTIVE.has(j.status)).length>=MAX_ACTIVE_JOBS) throw new Error(`目前已有 ${MAX_ACTIVE_JOBS} 個待處理研究，請等待或取消後再新增。`);
    ...
    const experienceContext=this.experiences.contextFor(input.topic);
    const experienceApplied={preferences:experienceContext.preferences.length,related:experienceContext.related.length};
    const readingContext=await this.readingMemory.contextFor(input.topic,this.config.sourceRoots);
    const readingMemoryApplied={records:readingContext.records.length,staleCount:readingContext.staleCount};
    const job={id,dir,...input,experienceContext,experienceApplied,readingContext,readingMemoryApplied,status:'queued',createdAt:now(),updatedAt:now(),progress:{stage:'queued',label:'等待研究'},events:[],error:null,stats:{sources:0,webSources:0,figures:0},report:{available:false,markdownUrl:`/api/jobs/${id}/report.md`,htmlUrl:`/api/jobs/${id}/report`},pendingRequests:[],runtimeEvidence:{cuaCalls:0,screenshotCalls:0,screenshotsSaved:0,webSearchCalls:0,webUrls:[]},_pending:new Map(),_save:Promise.resolve()};
  ...
  async refreshReadingContext(job) {
    const readingContext=await this.readingMemory.contextFor(job.topic,this.config.sourceRoots);
    job.readingContext=readingContext;
    job.readingMemoryApplied={records:readingContext.records.length,staleCount:readingContext.staleCount};
    await writeFile(path.join(job.dir,'work/workflow.txt'),developerInstructions(this.config,job),{mode:0o600});
  }
    ...
    try {
      ...
      for (;;) {
        ...
        try { await this.run(next); }
          ...
          if (!ACTIVE.has(next.status)) { await this.experiences.capture(next); await this.persist(next); }
        }
      }
  ...
  async run(job) {
    ...
    await this.refreshReadingContext(job);
    ...
    try {
      ...
      const started=await rpc.request('thread/start',{
        model:job.model, config:{...(job.reasoningEffort ? {model_reasoning_effort:job.reasoningEffort} : {}),web_search:job.sourceMode==='pdf_web'?'live':'disabled'},
      ...
      this.event(job,'complete',`報告已儲存並驗證：${job.stats.sources} 份 PDF、${job.stats.webSources || 0} 個網頁來源、${job.stats.figures} 張原圖。${job.status==='partial'?'請查看報告中的限制。':''}`);
    } finally {
      job._runEnded=true;
  ...
  async handleNotification(job,{method,params:p={}}) {
    ...
    const item=p.item || {};
    if (item.type==='webSearch' || item.type==='web_search_call') {
      const successful=item.status==='completed' && !item.error;
      if(successful)job.runtimeEvidence.webSearchCalls=(job.runtimeEvidence.webSearchCalls || 0)+1;
      const urls=successful && item.action?.type==='openPage' ? [item.action?.url].map(canonicalHttps).filter(Boolean) : [];
      if(urls.length)job.runtimeEvidence.webUrls=[...new Set([...(job.runtimeEvidence.webUrls || []),...urls])].slice(-500);
      await appendFile(path.join(job.dir,'work/web-trace.jsonl'),JSON.stringify({
        at:now(),id:item.id || null,query:typeof item.query==='string'?item.query.slice(0,2000):null,
        actionType:typeof item.action?.type==='string'?item.action.type:null,status:item.status || null,successful,url:urls[0] || null,
  ...
  async handleServerRequest(job,request) {
    ...
    if (request.method==='item/tool/call') {
      ...
      try {
        ...
        if (p.tool==='research_progress') {
          ...
          output={saved:true};
        } else if (p.tool==='research_record_reading') {
          const record=await this.readingMemory.record(job,args,this.config.sourceRoots);
          this.event(job,'reading',`已記錄《${record.source.title}》p.${record.pages.join('、')}，主題：${record.topics.join('、')}。`);
          output={saved:true,readingId:record.id,totalForJob:this.readingMemory.countForJob(job.id)};
    ...
    if (request.method==='item/tool/requestUserInput' || request.method==='tool/requestUserInput') {
      ...
      if(pdfSearchConsent){
        rpc.respond(request.id,{action:'accept',content:{}});
        this.event(job,'permission','已依 PDF Research 的永久設定允許本次操作 PDF Search。');
        return;
      }
      const id=String(request.id);
      job._pending.set(id,{kind:'approval',request});
      job.pendingRequests.push({id,kind:'approval',title:p.reason || p.message || 'Codex 需要操作授權',description:[p.command,p.cwd?`工作目錄：${p.cwd}`:'',p.permissions?JSON.stringify(p.permissions):'',p.requestedSchema&&Object.keys(p.requestedSchema.properties||{}).length?JSON.stringify(p.requestedSchema):''].filter(Boolean).join('\n').slice(0,6000)});
    } else {
  ...
  async verifyArtifacts(report) {
    const input=JSON.stringify({sources:report.sources.filter(source=>source.kind!=='web'),figures:report.figures,pdfinfo:path.join(this.config.pdfToolDir,'pdfinfo')});
  ...
  async publish(job,reportPath) {
    ...
    const result=await validateReport(report,{jobDir:job.dir,allowedSourceRoots:this.config.sourceRoots,sourceTarget:job.sourceCount,figureTarget:job.figureTarget,requireEvidence:true,allowWebSources:job.sourceMode==='pdf_web'});
    ...
    if (job.sourceMode==='pdf_web') {
      if ((job.runtimeEvidence.webSearchCalls || 0)===0) result.errors.push('本次選擇 PDF + 網路，但沒有觀察到 Codex 即時 Web Search。');
      const opened=new Set((job.runtimeEvidence.webUrls || []).map(canonicalHttps).filter(Boolean));
      for (const source of result.report?.sources || []) if (source.kind==='web' && !opened.has(canonicalHttps(source.url))) result.errors.push(`網頁來源 ${source.id} 沒有對應到本次 Web Search 實際開啟的 URL。`);
    }
    ...
    cancelled();
    const captured=await this.readingMemory.captureReport(job,result.report,this.config.sourceRoots);
    job.validation.valid=true;
    job.report.available=true;
    job.status='running';
    this.event(job,'saved',`已驗證並儲存 ${result.stats.sources} 份 PDF、${result.stats.webSources || 0} 個網頁來源、${result.stats.figures} 張圖；閱讀記憶新增或更新 ${captured.length} 筆。`);
    await this.persist(job);
  ...
  async saveFeedback(id, body) {
    const job=this.get(id);
    const experience=await this.experiences.saveFeedback(job,body);
    await this.persist(job);
    return experience;
  }
